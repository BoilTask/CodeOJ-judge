 ...
 * 
 * This file is part of CODEOJ.
 *
 * CODEOJ is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation; either version 2 of the License, or
 * (at your option) any later version.
 *
 * CODEOJ is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with CODEOJ. if not, see <http://www.gnu.org/licenses/>.
 */
#include <time.h>
#include <stdio.h>
#include <string.h>
...
void call_for_exit(int s) {
	if(DEBUG){
		...
		printf("CODEOJ Refusing to stop...\n Please use kill -9 !\n");
	}
}
...
void init_mysql_conf() {
	...
	if (fp != NULL) {
		...
		sprintf(query,
				"SELECT status_id FROM status WHERE language in (%s) and result<2 and MOD(status_id,%d)=%d ORDER BY result ASC,status_id ASC limit %d",
...
bool check_out(int status_id, int result) {
    char sql[BUFFER_SIZE];
	sprintf(sql,
			"UPDATE status SET result=%d,time=-1,memory=-1,score=0,judge_time=NOW() WHERE status_id=%d and result<2 LIMIT 1",
			result, status_id);
	if (mysql_real_query(conn, sql, strlen(sql))) {
	...
	for (int j = 0; jobs[j] > 0; j++) {
		...
		if(i<max_running){
			if (workcnt < max_running && check_out(runid, OJ_CI)) {
				...
				if (ID[i] == 0) {
					if (DEBUG){
						write_log("Judging status %d", runid);
